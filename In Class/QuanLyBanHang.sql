...
SELECT HOADON.SOHD FROM HOADON, CTHD
WHERE HOADON.SOHD = CTHD.SOHD AND MASP IN ('BB01', 'BB02') AND (SL BETWEEN 10 AND 20)

-- 13. Tìm các số hóa đơn mua cùng lúc 2 sản phẩm có mã số “BB01” và “BB02”, mỗi sản phẩm mua với số lượng từ 10 đến 20.
(SELECT HOADON.SOHD FROM HOADON, CTHD
WHERE HOADON.SOHD = CTHD.SOHD AND MASP = 'BB01' AND (SL BETWEEN 10 AND 20))
UNION
(SELECT HOADON.SOHD FROM HOADON, CTHD
WHERE HOADON.SOHD = CTHD.SOHD AND MASP = 'BB02' AND (SL BETWEEN 10 AND 20))
...
WHERE SANPHAM.MASP = CTHD.MASP AND HOADON.SOHD = CTHD.SOHD AND NGHD = '2007-01-01')
...
SELECT MASP, TENSP FROM SANPHAM
WHERE MASP NOT IN ( SELECT MASP FROM CTHD CT, HOADON HD 
					WHERE HD.SOHD=CT.SOHD AND YEAR(NGHD)=2006)

-- 17. In ra danh sách các sản phẩm (MASP,TENSP) do “Trung Quoc” sản xuất không bán được trong năm 2006.
SELECT MASP, TENSP FROM SANPHAM
WHERE NUOCSX='Trung Quoc' AND MASP NOT IN ( SELECT MASP FROM CTHD CT, HOADON HD 
											WHERE HD.SOHD=CT.SOHD AND YEAR(NGHD)=2006)

-- 18. Tìm số hóa đơn trong năm 2006 đã mua ít nhất tất cả các sản phẩm do Singapore sản xuất.
SELECT SoHD FROM HOADON HD
WHERE not exists ( Select * From SANPHAM SP	
					WHERE NUOCSX='Singapore' and Not exists ( Select * From CTHD CT
